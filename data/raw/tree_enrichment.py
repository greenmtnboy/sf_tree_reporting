...
SYNONYMS = {'albizia distachya': 'paraserianthes'}
...
def enrich_species(q_species: str, client) -> TreeEnrichment | None:
    ...
    if not scientific_name:
        return None
    text = fetch_wikipedia_text(scientific_name)
    if not text:
        print(f"  [skip] no Wikipedia content for {scientific_name!r}", file=sys.stderr)
        return None
    try:
...
def get_all_species() -> list[str]:
    """Return all distinct q_species values from the tree dataset."""
    conn = duckdb.connect()
    try:
        rows = conn.execute(
            "SELECT DISTINCT q_species FROM read_parquet(?) WHERE plant_type= 'Tree' AND q_species IS NOT NULL ORDER BY q_species",
            [TREE_INFO_PARQUET],
        ).fetchall()
